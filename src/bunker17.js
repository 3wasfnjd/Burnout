...
scene.fog = new THREE.FogExp2(0x121619, 0.008);
...
const emergency = new THREE.PointLight(0xff4938, 0.8, 8, 2);
emergency.position.set(0, 3.45, 0.5); room.add(emergency);
const ambient = new THREE.HemisphereLight(0xdde8ed, 0x30271f, 0.9); scene.add(ambient);
const fillLight = new THREE.DirectionalLight(0xd9edf5, 1.0); fillLight.position.set(2.5,5.5,4.5); scene.add(fillLight);
const warmFill = new THREE.DirectionalLight(0xffc994, 0.65); warmFill.position.set(-4.5,3.5,-1.5); scene.add(warmFill);
const mainLights=[];
for(const p of [[-3.8,3.25,0],[0,3.25,0],[3.8,3.25,0]]){const l=new THREE.PointLight(0xffd8ad,2.0,11,1.65);l.position.set(...p);l.castShadow=false;room.add(l);mainLights.push(l);}
...
function solve(n){completed[n]=true;if(n===0){emergency.intensity=.35;ambient.intensity=1.0;mainLights.forEach(l=>l.intensity=3.2);wallWashLights.forEach(l=>l.intensity*=1.2);}if(n===1)consoleGlow.intensity=.8;if(n===3){doorLight.color.set(0xffb12f);doorLight.intensity=3;}if(n===4){doorLight.color.set(0x55ff9a);doorLight.intensity=4.5;}if(n<4)setStage(n+1);else if(hint)hint.textContent='تم فتح BUNKER 17 — الحرية أمامك';}
function nearestStation(){let best=null,dist=999;for(const s of stations){const d=player.position.distanceTo(s.pos);if(d<dist){dist=d;best=s;}}return dist<2.6?best:null;}
